...
object Main {
  private def navigateMap(map: Array[Array[Char]]): Either[String, MapResult] = {
    findStartPosition(map) match {
      ...
      case Right((startRow, startCol)) =>
        ...
        var (rowDelta, colDelta) = initialDirection(startRow, startCol, map)
        ...
        while (!finished) {
          ...
          if (currentChar != ' ') {
            ...
            currentChar match {
              ...
              case _ if currentChar.isLetter || currentChar == '+' =>
                ...
                println(s"Direction before '+' processing: (rowDelta=$rowDelta, colDelta=$colDelta)")

                // Create a sequence of possible directions with the current direction prioritized
                ...
                println(s"Direction after '+' processing: (rowDelta=$rowDelta, colDelta=$colDelta)")
              case '-' =>
          ...
          if (currentChar == 'x') {
            ...
            // Print delta values right before the calculation
            println(s"Delta values: (rowDelta=$rowDelta, colDelta=$colDelta)")
            // Update position
            val newRow = row + rowDelta
            val newCol = col + colDelta

            // Print new position after calculation
            println(s"New position to move: (newRow=$newRow, newCol=$newCol)")
  ...
  private def initialDirection(startRow: Int, startCol: Int, map: Array[Array[Char]]): (Int, Int) = {
...
//        }
    ...
    val result = navigateMap(examples.validExamples(mapN).map)
    val testPassed = result == examples.validExamples(mapN).expectedResult
    println(s"Test passed: $testPassed - Result: $result, Expected: ${examples.validExamples(mapN).expectedResult}")
  }
}
